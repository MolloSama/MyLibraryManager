...
bool MainWindow::dbCreat()
{
    if(dbConnect()){
        ...
        BkModel->setHeaderData(BkId, Qt::Horizontal, "编号");
        ...
        BkModel->setHeaderData(BkPubDate, Qt::Horizontal, "出版社号");
        ...
        LoanModel->setHeaderData(LBkId,Qt::Horizontal,"编号");
...
void MainWindow::on_pushButton_9_clicked()
{
    ...
    else
    {
        ...
        UsrLoanModel->setHeaderData(LBkId,Qt::Horizontal,"编号");
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    QString bkstore=BkModel->record(curRow).value(BkStore).toString();
    QString bkname=BkModel->record(curRow).value(BkName).toString();
    QString author=BkModel->record(curRow).value(BkAuthor).toString();
    QString pub=BkModel->record(curRow).value(BkPub).toString();
    QString pubdate=BkModel->record(curRow).value(BkPubDate).toString();
    QString avail=BkModel->record(curRow).value(BkAvail).toString();
    QString total=BkModel->record(curRow).value(BkTotal).toString();
    bool okk;
    int dec=bkstore.toInt(&okk,10);
    dec--;
    bkstore=QString::number(dec,10);
    ...
    QSqlRecord record2 = BkModel->record();
    record2.setValue("id",book);
    record2.setValue("name",bkname);
    record2.setValue("store",bkstore);
    record2.setValue("author",author);
    record2.setValue("pub",pub);
    record2.setValue("pub_date",pubdate);
    record2.setValue("avail",avail);
    record2.setValue("total",total);
    BkModel->insertRecord(curRow+1,record2);

    BkModel->removeRow(curRow);

    int ok=QMessageBox::warning(this,"Borrow","确定借阅吗",
                                QMessageBox::Yes,QMessageBox::No);
    if(ok==QMessageBox::No)
    {
        LoanModel->revertAll();
...
void MainWindow::on_pushButton_15_clicked()
{
    ...
    int ok=QMessageBox::warning(this,"Return","确定要还书吗？",
                                QMessageBox::Yes,QMessageBox::No);
    if(ok==QMessageBox::No)
    {
...
void MainWindow::on_pushButton_14_clicked()
{
    ...
    int ok=QMessageBox::warning(this,"Continue","确定要续借吗？",
                                QMessageBox::Yes,QMessageBox::No);
    if(ok==QMessageBox::No)
    {
        UsrLoanModel->revertAll();
